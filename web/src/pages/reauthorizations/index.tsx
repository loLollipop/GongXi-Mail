...
import { CopyOutlined, PauseCircleOutlined, PlayCircleOutlined, ReloadOutlined } from '@ant-design/icons';
...
export default function ReauthorizationsPage() {
    ...
    }, [running, busy, loaded, session, queue, check, start]);
            ...
            <Col xs={24} xl={12}>
                <Card title="当前授权" extra={session && <Tag color={session.status === 'SUCCEEDED' ? 'success' : active ? 'processing' : 'warning'}>{statusLabels[session.status]}</Tag>}>
                    {session ? <Space orientation="vertical" size="large" style={{ width: '100%' }}>
                        <Typography.Title level={4} style={{ margin: 0, overflowWrap: 'anywhere' }}>{session.email}</Typography.Title>
                        ...
                        {active && <Button type="primary" size="large" block icon={<CopyOutlined />} disabled={!session.userCode || secondsLeft <= 0} onClick={openMicrosoft}>复制验证码并打开微软授权页面</Button>}
                        ...
                        <Space wrap>
                            <Button icon={running ? <PauseCircleOutlined /> : <PlayCircleOutlined />} disabled={!running && !active && counts.waiting === 0} onClick={() => setRunning((value) => !value)}>{running ? '暂停' : '继续'}</Button>
                            <Button loading={busy} disabled={!active || busy || waitSeconds > 0} onClick={() => void check()}>{waitSeconds > 0 ? `${waitSeconds} 秒后可检查` : '立即检查'}</Button>
                            <Button disabled={busy || session.status === 'SUCCEEDED' || session.status === 'CANCELLED'} onClick={() => void skip()}>跳过</Button>
                            <Button disabled={busy || !canRetry} onClick={retry}>重试当前账号</Button>
                        </Space>
                        ...
                        <Button type="primary" size="large" loading={busy} disabled={!loaded || !counts.waiting} onClick={() => setRunning(true)}>开始批量授权</Button>
                    </Space>}
                </Card>
            </Col>
        </Row>
